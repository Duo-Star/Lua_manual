...
function main()
  local F=合力(作用力(定点,杆,球),重力(球))
  作用于(球,F)
end

--]]
--怎么样？是不是很有趣？
--本条目将以双摆为例,带你走进nature物理环境
...
定点=物体({x=vector(0,0)})
杆=杆({l=1, k=30000})
球a=物体({x=vector(sqrt(2)/2,sqrt(2)/2)})
球b=物体({x=vector(sqrt(2)/2,sqrt(2)/2+1)})

function main()
  local Fa=作用力(定点,杆,球a)
  local Fb=作用力(球a,杆,球b)
  作用于(球a,合力(重力(球a),合力(Fa,相互作用力(Fb))))
  作用于(球b,合力(重力(球b),Fb))

  tips.setText("球:("..保留小数(球a.x.x,0.01,1)..","..保留小数(球a.x.y,0.01,1)..")")
end
...
function(canvas, paint, panel)
  ...
  function line_(x0,y0,x1,y1,p)
    ...
    canvas.drawLine(x0*Lambda+ox,-y0*Lambda+oy,x1*Lambda+ox,-y1*Lambda+oy, p)
  end
  dot(球a.x.x*100,球a.x.y*100)
  dot(球b.x.x*100,球b.x.y*100)
  line_(0,0,球a.x.x*100,球a.x.y*100)
  line_(球a.x.x*100,球a.x.y*100,球b.x.x*100,球b.x.y*100)

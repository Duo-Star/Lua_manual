...
init.main=function()
  ...
  function main()
    for m=1,#particle do
      for n=1,#particle do
        if m~=n then
          local f=force(particle[n], g_, particle[m])
          particle[m].a =particle[m].a+ (1/particle[m].m) * f


        end
      end
      object.update(particle[m])
      particle[m].a=vector()
      Data[m]=geo(to.point(particle[m].x),{tag="A",width=12,color=0xeeFFFEFE,sun={true,16,0,0,0xFFFFEE58}})
    end
  end

--20250730修复切后台或锁屏重进后页面变黑
--20250727感谢qq928182278：用lockHardwareCanvas流畅多了
--效率很低
--也许是lua-java耗费了太长时间？
--或者说没有直接调用gpu？
...
**]]
  ...
  paint坐标轴=Paint().setColor(0x88252525).setStyle(Paint.Style.STROKE).setStrokeWidth(4).setAntiAlias(true).setStrokeCap(Paint.Cap.ROUND)
  ...
  holder.addCallback(SurfaceHolder.Callback {
        ...
        --]]
            ...
            for a=-pi,pi,0.15 do
              ...
              for b=-pi,pi,0.15 do
                ...
                --data[#data+1]=new
                --d3.drawLine_by2p(canvas,old,new,paint坐标轴)
                d3.drawPoint(canvas,new,paint坐标轴)
                old=new
              end
            end
            --[[
    ...
    --]]
            ...
            d3.drawLine_by2p(canvas,d3.change(d3.p),d3.change(d3.p+Vector(1)),paint坐标轴)
            d3.drawLine_by2p(canvas,d3.change(d3.p),d3.change(d3.p+Vector(0,1)),paint坐标轴)
            d3.drawLine_by2p(canvas,d3.change(d3.p),d3.change(d3.p+Vector(0,0,1)),paint坐标轴)
  ...
  surface.onTouch =function(v, event)
    ...
    if(event.getActionMasked() == MotionEvent.ACTION_DOWN)
      if d3.debugMode print_("手指碰到屏幕") end
      d3.o_0=d3.o
      ...
      for a, item in pairs(data) do
        ...
        if class == Particle then
          local dx=gtp-item.x
          if (#dx)*d3.lam<50 then--检测被触摸
            d3.select[1]=a--向选择对象表中添加此对象
            --[[
          select_card_dimiss(false)
          local sp=d3.toSP(item.x)--该对象在屏幕上的位置
          select_card.x=sp.x
          select_card.y=sp.y

...
btn.onClick=function(n)
  ...
  app1.evalCommand("x^2+y^2=1");
  app1.evalCommand("y=0");
  app1.evalCommand("x=1");
  ]],nil)
  timer_.start()
end
...
dt=0.08
timer_=Ticker()
timer_.Period=dt*1000
timer_.onTick=function()
  t=t+dt
  web.evaluateJavascript([[
    app1.evalCommand("A=(]]..math.cos(t)..[[,]]..math.sin(t)..[[)");
    app1.evalCommand("a=(]]..math.cos(t)..[[,]]..math.sin(t)..[[)");
    app1.evalCommand("B=(1,]]..math.sin(t)..[[)");
    app1.evalCommand("f(x)=If(x>1, sin(pi-1+x-]]..t..[[))");
    app1.evalCommand("h(x)=If(cos(]]..t..[[)<x<1, sin(]]..t..[[))");
...
web.addJSInterface(LuaWebView.JsInterface{
...
--web.loadUrl([[]],nil)
--web.evaluateJavascript([[lua.execute("print(1)")]],nil)
...
mimeType="text/html"
enCoding="utf-8"
web.loadDataWithBaseURL(nil,html,mimeType,enCoding,nil)
